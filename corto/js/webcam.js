...
window.webcam = {
	...
	get_movie: function() {
		...
		parent.ValidaCamara();
		//alert("ERROR: No tienes una camara conectada, por favor conecta una o selecciona una foto de tu quipo local");
	...
	freeze: function() {
		...
		$('#CamaraOK').css('visibility','visible');
		$('#FotoLocalOK').css('visibility','hidden');
		$('#TxtTipoFoto').val('camara');		
	},
	
	freeze_stop: function() {
		// freeze webcam image (capture but do not upload)
		this.get_movie()._snap('', this.quality ? 1 : 0, 0 );		
	},
		...
		switch (type) {
			...
			case 'error':
				...
				if (!this.fire_hook('onError', msg)) {
					//alert("Captura una imagen con tu camara local o sube una desde tu equipo local");
				}
				break;
			...
			case 'success':
				...
				this.fire_hook('onComplete', msg.toString());				
						...
						if (msg.match(/(http\:\/\/\S+)/)) {
							...
							'<form id="FrmDatos" name="FrmDatos" action="querys.php?opcion=guardar_informacion_camara" method="post">'+	
							'<input type="hidden" name="camara_id" id="camara_id" value="' + $('#TxtFile').val() + '"  /><br>'+		
							'<input type="hidden" name="camara_nombre" id="camara_nombre" value="' + $('#TxtNombre').val() + '"/>'+
							'<input type="hidden" name="camara_email" id="camara_email" value="' + $('#TxtEmail').val() + '"/></form>';
							$('#FrmDatos').submit();													
						}																								
				break;
